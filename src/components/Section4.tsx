    ...
    <main className='bg-gray-200 px-52 py-20 h-max'>
        <section className='flex gap-2 justify-between'>         
        {
            services.map((item,index) =>(
                <div className={` flex flex-col rounded-md text-wrap py-10  w-1/4 pl-3 ${index === 0 ? 'bg-black text-white' : 'bg-white text-black'}`} key={index}> 
                <div className="flex-1 flex flex-col gap-5">

                <p className='uppercase font-bold'>{item.p1}</p>
                <h4 className='text-3xl capitalize font-bold'>{item.h4}</h4>
